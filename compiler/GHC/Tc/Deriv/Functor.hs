...
mkSimpleLam lam =
    get >>= \case
      n:names -> do
        put names
        body <- lam (nlHsVar n)
        return (mkHsLam [LamVisPat (nlVarPat n)] body)
      _ -> panic "mkSimpleLam"
...
mkSimpleLam2 lam =
    get >>= \case
      n1:n2:names -> do
        ...
        return (mkHsLam (LamVisPat <$> [nlVarPat n1,nlVarPat n2]) body)
...
mkSimpleConMatch ctxt fold extra_pats con insides = do
    ...
    return $ mkMatch ctxt (LamVisPat <$> (extra_pats ++ [pat])) rhs emptyLocalBinds
    ...
    let con_name = getRdrName con
        ...
        con_expr
          ...
          | otherwise =
              ...
              in mkHsLam (map (LamVisPat . nlVarPat) bs) (nlHsApps con_name vars)
...
gen_Foldable_binds loc tycon _
  ...
  where
    ...
    foldMap_eqns = [mkSimpleMatch foldMap_match_ctxt
                                  [LamVisPat nlWildPat, LamVisPat nlWildPat]
                                  mempty_Expr]
    ...
    null_eqn con
      = flip evalState bs_RDRs $ do
          ...
          case convert parts of
            Nothing -> return $
              mkMatch null_match_ctxt [LamVisPat (nlParPat (nlWildConPat con))]
  ...
  where
    ...
    traverse_eqns =
        [mkSimpleMatch traverse_match_ctxt
                       [LamVisPat nlWildPat, LamVisPat z_Pat]
